...
const portMap: Record<string, number> = {
  designer: 5173, // 设计端（包含设计器和资源管理界面）
  admin: 5174, // 管理端（独立的中后台管理系统）
  mobile: 5175,
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      VitePWA({
        ...
        workbox: {
          ...
          globPatterns: [
            '**/*.{js,css,html}', // 默认规则
            '**/*.{png,jpg,jpeg,gif,svg}', // 添加图片
            '**/*.{mp4,webm,ogg}', // 添加视频
            '**/*.{otf,eot,ttf,webp}', // 添加字体
          ],
          navigateFallbackDenylist: [
            /^\/api/, // 排除 /api 开头的路径
          ],
          maximumFileSizeToCacheInBytes: 50 * 1024 * 1024, // 10 MiB
          runtimeCaching: [
            {
              ...
              options: {
                ...
                expiration: {
                  maxEntries: 1000,
                  maxAgeSeconds: 360 * 24 * 60 * 60, // 7 days
                },
                cacheableResponse: {
                  statuses: [0, 200],
                },
    ...
    server: {
      ...
      open: env.VITE_APP_MODEL === 'designer' ? '/designer/login' : env.VITE_APP_MODEL === 'admin' ? '/admin/login' : true,
      fs: {
        strict: false,
        allow: ['..'],
      },
    ...
    build: {
      ...
      rollupOptions: {
        ...
        output: {
          ...
          assetFileNames: ({ name }) => {
            if (name != null && /\.css$/.test(name)) {
              return 'assets/css/[name].[hash][extname]'
            } else if (name != null && /\.(png|jpg|jpeg|gif|svg)$/.test(name)) {
